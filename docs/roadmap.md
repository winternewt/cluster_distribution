...
- **§2 (eps-independent scorer): DONE** (master corrected 2026-06-07, see `RCA.md` §7). The `R·eps^α(ε)` collapse (α(ε)=2.031525+0.258273·ln ε; fit from 110 eps values) cuts the residual spread to <0.5%. Master: **shifted inverse-gamma, integer shape 10** (= min_samples), `SF(R̃)=P(10, 157.70/(R̃−7.51))` — pooled KS≈0.005 over eps 1.00–1.60, |median_z|≤0.02 per eps. The earlier zero-loc masters (log-logistic 24.06/7.87, inv-gamma shape≈20.5) mis-centre z by ~0.07σ and are deprecated. Same constants in `analysis/scorer.py`/`scorer_master.json`, `modules/cluster_detector.py` (`master_sf`, `score_clusters`), and `webapp/master.js`. The eps-weighting debate is moot.
...
- **New TODOs from the master correction (2026-06-07, `RCA.md` §7–8):** (e) **min_samples sweep** (cheap sim, one eps): floor should move as `m·(1+6.6/(m−1)²)`; if the fitted master *shape* also tracks m, shape=min_samples is physics, else it's the anchored-effective-exponent coincidence; (f) `(n, eps)`-conditional master (per-N′ shifted inv-gammas measure 2× better, KS(z) 0.0024 vs 0.0043) to erase the occupancy-drift residual; (g) refit α(ε) *jointly* with the shifted master (it was fit to collapse zero-loc medians; shift and rescale don't commute — the free-fit loc drift 7.5→6.8 over eps 1.10→1.40 is the fingerprint). Done since: N-invariance at fixed λ₀ verified (`analysis/ncheck.py`).
