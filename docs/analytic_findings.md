...
> 4. **Scorer (byproduct).** One master on `R̃=R·eps^α(ε)` gives a validated, eps-independent rare-event score — dissolving the eps-weighting ambiguity the old handoffs agonized over. The running exponent `α(ε)=2.031525+0.258273·ln ε` (fit from 110 eps values) cuts the residual spread from ~5% (naive eps²) to <0.5%. Parametric master (corrected 2026-06-07, `RCA.md` §7): **shifted inverse-gamma, integer shape 10 = min_samples**, `SF(R̃)=P(10, 157.70/(R̃−7.51))` — pooled KS≈0.005 over eps 1.00–1.60, |median_z|≤0.02 per eps. The location shift is essential (`R|n` is bounded away from 0); the earlier zero-loc masters (log-logistic 7.87/24.06, inv-gamma shape≈20.5) mis-centre z by ~0.07σ and are deprecated. Shape 10 = the `a→∞` limit of the legacy Beta-Prime(a≈46, b≈10) fits. `analysis/scorer.py`, `scorer_table.csv`, `scorer_master.json`.
...
> 6. **The master decoded + N-invariance (2026-06-07, `RCA.md` §8).** The master's support floor is DBSCAN's *certification bound* — an `n=min_samples` cluster must fit in one core's eps-ball ⇒ `R̃ ≥ m·(1+2π²/3(m−1)²) ≈ 10.87` (observed global min 10.92 over 1.2M clusters); its mean `loc+scale/(shape−1)=25.03` equals the derived `C≈25` of point 3. The law is **N-invariant at fixed λ₀** (`analysis/ncheck.py`, N=10k/20k/40k, radius √N: identical master & calibration, yield ∝ N) — `R̃` is a purely local observable of (λ₀, eps, min_samples).
>
...
2. The eps-independent scorer (T4/§byproduct) follows immediately: score any observed cluster by mapping `R̃ = R·eps^α(ε)` with `α(ε)=2.031525+0.258273·ln ε` and reading the tail of the single master distribution — the shifted inverse-gamma `SF(R̃) = P(10, 157.70/(R̃−7.51))`, shape 10 = min_samples (the location shift is what the earlier zero-loc log-logistic missed; `RCA.md` §7). The running exponent reduces the residual spread from ~5% (naive eps²) to <0.5%. No eps-mixture weighting needed.
